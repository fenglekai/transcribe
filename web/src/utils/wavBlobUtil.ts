/**
 * fork: https://github.com/upadhyaypushpendra/webm-to-wav-converter
 */
function _writeStringToArray(aString: string, targetArray: Uint8Array<ArrayBuffer>, offset: number) {
  for (let i = 0; i < aString.length; ++i)
      targetArray[offset + i] = aString.charCodeAt(i);
}

function _writeInt16ToArray(aNumber: number, targetArray: Uint8Array<ArrayBuffer>, offset: number) {
  aNumber = Math.floor(aNumber);
  targetArray[offset + 0] = aNumber & 255;          // byte 1
  targetArray[offset + 1] = (aNumber >> 8) & 255;   // byte 2
}

function _writeInt32ToArray(aNumber: number, targetArray: Uint8Array<ArrayBuffer>, offset: number) {
  aNumber = Math.floor(aNumber);
  targetArray[offset + 0] = aNumber & 255;          // byte 1
  targetArray[offset + 1] = (aNumber >> 8) & 255;   // byte 2
...
function _writeAudioBufferToArray(
  ...
  targetArray: Uint8Array<ArrayBuffer>,
  offset: number,
  bitDepth: number
) {
...
async function _getAudioBuffer(blobData: BlobPart, contextOptions = undefined) {
  let blob = blobData;
...
async function getWaveBlob(
  blobData: BlobPart, as32BitFloat: boolean, contextOptions = undefined
) {
